...
from scipy.optimize import minimize  # 用于求解优化问题（经济调度）
from scipy.ndimage import gaussian_filter1d  # 用于平滑曲线（绘图时使用）
import tkinter as tk  # GUI基础库
from tkinter import ttk, messagebox  # 扩展GUI组件和消息框
from collections import deque  # 双端队列，用于高效存储历史数据
import warnings
...
# 小波包分解（优先使用pywt）
try:
    import pywt  # PyWavelets库，用于小波分解
    PYWT_AVAILABLE = True
except ImportError:
    PYWT_AVAILABLE = False
    print("⚠️ PyWavelets 未安装，使用移动平均近似小波分解（频带分离精度稍降）")

# 碳排放因子 (kg CO₂/kWh) - 用于计算碳减排
CARBON_FACTOR = 0.5
# 碳价格 (元/kg CO₂) - 用于环保模式，在目标函数中加入碳惩罚成本
CARBON_PRICE = 0.05

# ==================== 配色方案 ====================
# 定义一套统一的颜色，用于GUI和图表，增强视觉效果
...
# ==================== 全局配置参数 ====================
SOC_MIN = 0.1  # 储能设备荷电状态下限
SOC_MAX = 0.9  # 储能设备荷电状态上限
FILTER_ALPHA_DEFAULT = 0.1  # 默认低通滤波系数，用于平滑电流/电压
TRACK_TOLERANCE = 0.05  # 二次功率调整允许的最大偏差 (MW)，确保跟踪精度

# 分时电价（南京冬季工商业，单位：元/kWh）
PRICE_VALLEY = 0.21  # 低谷电价
PRICE_FLAT = 0.62    # 平段电价
PRICE_PEAK = 1.12    # 高峰电价
...
class EnergyStorageDevice:
    ...
    def __init__(self, name, power_rating, capacity,
        ...
        初始化储能设备的基本参数
        :param name: 设备名称
        :param power_rating: 额定功率 (MW)
        :param capacity: 额定容量 (MWh)
        :param charge_eff: 充电效率
        :param discharge_eff: 放电效率
        :param op_cost: 运维成本 (元/MWh)
        :param response_time: 响应时间 (秒)，用于斜坡限制
        :param aging_coeff: 老化成本系数 (元/MW^1.8/h)
        :param gas_price: 天然气价格 (元/m³)，仅压缩空气储能使用
        """
        self.name = name
        ...
        self.soc = 0.8
        self.current_power = 0.0  # 当前实际功率 (MW)
        self.voltage = 0.0         # 当前端电压 (V)
        self.current = 0.0         # 当前电流 (A)
        ...
        self.history_fuel_cost = deque(maxlen=10000)  # 燃料成本（仅CAES）
    ...
    def update_state(self, power_set, dt):
        ...
        :param power_set: 设定的功率指令 (MW)
        :param dt: 仿真步长 (小时？实际上是秒，注意单位转换)
        :return: 更新后的SOC、总成本和储存能量
        """
        # 斜坡限制（功率变化率限制），防止功率突变
        ...
        # 能量更新：根据功率和效率计算实际充放电能量
        if power >= 0:  # 放电
            # 放电时，从储存能量中扣除的能量需除以放电效率，以反映内部损耗
...
class FESS(EnergyStorageDevice):  # 飞轮储能
    def __init__(self):
        ...
        self.voltage_nominal = 480  # 额定电压 480V
...
class SMES(EnergyStorageDevice):  # 超导磁储能
    def __init__(self):
        ...
        self.critical_current = 10000  # 临界电流限制 (A)
...
class CAES(EnergyStorageDevice):  # 压缩空气储能（采用多变过程模型）
    def __init__(self):
        ...
        self.p0 = 1.0  # 初始压力 (标幺)
        self.k = 1.4  # 空气绝热指数
        self.gas_consumption = deque(maxlen=10000)  # 记录燃气消耗
...
class HierarchicalMPC:
    """
    分层MPC控制器，包含上层经济调度和下层实时平衡。
    上层基于随机规划进行经济调度，下层使用小波分解进行频段分配。
    """
    def __init__(self, pred_horizon=10, control_horizon=5,
                 stochastic=True, n_scenarios=10, uncertainty_std=0.05):
        """
        初始化MPC参数
        :param pred_horizon: 预测时域长度 (步数)
        :param control_horizon: 控制时域长度
        :param stochastic: 是否启用随机规划
        :param n_scenarios: 随机场景数量
        :param uncertainty_std: 预测误差标准差 (相对值)
        """
        ...
        self._buffer = None  # 用于小波分解的数据缓冲区
    ...
    def wavelet_decompose(self, signal, levels=3, wavelet='db4'):
        ...
        if not PYWT_AVAILABLE:
            # 如果pywt未安装，使用移动平均近似代替小波分解
            low = np.convolve(signal, np.ones(10)/10, mode='same')  # 低频近似
            residual = signal - low
            mid = np.convolve(residual, np.ones(5)/5, mode='same')  # 中频近似
            high = residual - mid  # 高频分量
    ...
    def economic_dispatch(self, devices, net_load, time_idx, dt, eco_mode=False):
        ...
        :param time_idx: 当前时间索引 (用于计算电价)
        :param dt: 仿真步长 (秒)
        :param eco_mode: 是否开启环保模式（考虑碳惩罚）
        :return: 各设备的经济调度功率指令数组
        """
        n = len(devices)
        if n == 0:
            return np.array([])
        ...
        # 购电成本系数：将功率 (MW) 转换为 dt 内的购电费用 (元)
        const = dt / 3600 * price * 1000  # 元/MW（对应dt内）
    ...
    def _allocate_frequency_power(self, devices, power_target, freq_type, dt):
        """
        按频段分配功率（边际成本优先，但高频设备指定优先级）
        :param devices: 设备列表
        :param power_target: 该频段的目标功率 (MW)
        ...
        if freq_type == 'high':
            ...
            # 中频设备：飞轮储能、锂电池储能，按边际成本排序分配
            eligible = [d for d in devices if d.name in ['飞轮储能', '锂电池储能']]
            eligible.sort(key=lambda d: d.marginal_cost)  # 边际成本低的优先
            for dev in eligible:
                if abs(remaining) < 1e-6:
                    break
                charge_limit = -min(dev.power_rating, dev.get_charge_available() * 3600 / dt)
...
class HybridEnergyStorageSystem:
    ...
    def __init__(self):
        ...
        self.monitor_window = None  # 实时监控窗口
        self.monitor_labels = {}    # 存储监控标签的字典
    ...
    def _adjust_power_to_track(self, set_powers, devices, target, dt):
        """
        二次功率调整：若当前总功率与目标净负荷偏差超过TRACK_TOLERANCE，
        则按各设备剩余可调容量比例补充，以提高跟踪精度。
        """
        ...
        for i, dev in enumerate(devices):
            ...
            p_max = min(dev.power_rating, max_discharge)
            if diff > 0:
                room = p_max - set_powers[i]  # 可增加放电的空间
            else:
                room = set_powers[i] - p_min  # 可增加充电的空间
            ...
            if charge_indices:
                ...
                if total_abs > 0:
                    ...
                    for idx, red in zip(charge_indices, reductions):
                        set_powers[idx] = min(0, set_powers[idx] + red)  # 加上正数，功率向零移动
        return set_powers

    def _update_trend_graph(self, dev_name, power):
        """更新趋势图数据并重绘（在监控面板的小画布上）"""
        max_points = 50
    ...
    def simulate(self, pulse_amp, pulse_dur, plant_power=10.0, sim_time=30, dt=0.1, econ_interval=1, eco_mode=False):
        ...
        :param sim_time: 仿真总时长 (s)
        :param dt: 仿真步长 (s)
        :param econ_interval: 经济调度执行间隔 (步数)
        ...
        # 修改：脉冲起始时间改为100秒（位于总时间中间）
        start = int(100.0 / dt)
        end = int((100.0 + pulse_dur) / dt)
        load[start:end] += pulse_amp
        plant = plant_power * np.ones_like(time)  # 电厂恒发功率
        net_load = load - plant  # 净负荷（需储能平衡）
        ...
        last_econ_powers = np.zeros(len(self.devices))  # 上次经济调度结果
        econ_cnt = 0
    ...
    def _update_monitor(self, total_cost=None, grid_power=None, init=False):
        ...
        :param total_cost: 当前总成本，用于更新标签
        :param grid_power: 当前电网功率，用于更新标签
        :param init: 是否为初始化模式（创建窗口）
        """
        if init:
            # 如果已有监控窗口，先销毁
...
class HESSGUI:
    ...
    def __init__(self):
        ...
        # 用户输入变量
        self.pulse_amp = tk.DoubleVar(value=12.0)   # 脉冲幅值
        self.pulse_dur = tk.DoubleVar(value=10.0)   # 脉冲持续时间
        self.plant_power = tk.DoubleVar(value=10.0) # 电厂恒发功率
        # 设备选择变量
        self.dev_vars = {
        ...
        self.use_stochastic = tk.BooleanVar(value=True)  # 是否启用随机规划
        self.n_scenarios = tk.IntVar(value=10)           # 场景数量
        self.uncertainty_std = tk.DoubleVar(value=0.05)  # 预测误差标准差
